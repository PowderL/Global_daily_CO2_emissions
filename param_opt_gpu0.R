...
for (sector_i in c("Power", "Industry", "Transportation")) {
  ...
  dataset_all <- fread(paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/train_and_predict_dataset_", sector_i, "_1229.csv"))
  ...
  country_list <- c('United States', 'China', 'India', 'Japan', 'Brazil', 
                    'Germany', 'Italy', 'United Kingdom', 'Austria', 'Portugal', 
                    'Hungary', 'Spain', 'Poland', 'Croatia', 'Latvia', 
                    'Netherlands', 'Slovenia', 'France', 'Cyprus', 
                    
                    'Denmark', 'Finland', 'Malta', 'Lithuania', 
                    'Sweden', 'Belgium', 'Russian Federation', 'Luxembourg', 
                    'Slovakia', 'Greece',  'Bulgaria', 'Estonia', 
                    'Romania', 'Czech Republic', 'Ireland', 'ROW')
  
  gpu_id <- 0
  for (coun in c('United States', 'China', 'India', 'Japan', 'Brazil')) {
    ...
    fwrite(validation_eta_all, paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/parm_opt_1229/", tolower(sector_i), "_validation_for_", coun, ".gz"))
  }
