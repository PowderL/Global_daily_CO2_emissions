...
CM_data_daily <- CM_data[(!country %in% c("WORLD", "EU27 & UK", "United Kingdom", "Spain", "France", "Germany", "Italy")) & myear <= 2024 & sector != "International Aviation", .(CM = sum(value)), by = .(mdate, country, sector)]
...
CM_EU_data_daily <- CM_EU_data[(!country %in% c("WORLD", "UK", "Norway", "Switzerland")) & myear <= 2024, .(CM = sum(value)), by = .(mdate, country, sector)]
CM_EU_data_daily[country == "EU27 & UK", country := "EU"]

CM_Int_data_daily <- CM_data[country == "WORLD" & myear <= 2024 & sector == "International Aviation", .(CM = sum(value)), by = .(mdate, country, sector)]
CM_Int_data_daily[, country := "Int"]
CM_Int_data_daily[, sector := "Transportation"]
CM_plus_EU_data_daily <- rbindlist(list(CM_data_daily, CM_EU_data_daily, CM_Int_data_daily), use.names = T)
...
temperature_world <- fread("/data/ltao/ERA5/countries_population_weigted_mean_temperature_Global_new_pop.csv")
...
edgar_emission_dataset_dt <- melt(edgar_emission_dataset, id.vars = c("Name", 'Country_code_A3', "Year", "ipcc_code_1996_for_standard_report"), variable.name = "month_str", value.name = "co2_emission")
...
EDGAR_EIA_scale_factor <- fread("/data/ltao/Qiu_paper/美国工业部门EDGAR-EIA对齐比例因子.csv")
edgar_emission_dataset_dt[EDGAR_EIA_scale_factor, scale_factor := i.scale_factor, on = .(myear)]
edgar_emission_dataset_dt[country == "United States" & sector == "Industry", co2_emission_month_avg := co2_emission_month_avg * scale_factor]


## 
# "Int. Shipping" 
# "Int. Aviation" 
edgar_emission_dataset_dt[Name %in% c("Int. Aviation"), country := "Int"]

edgar_emission_dataset_dt[!Name %in% c(unique(temperature_global$country), "Int. Aviation", "Int. Shipping"), country := "ROW"]
...
edgar_emission_dataset_dt <- rbindlist(list(edgar_emission_dataset_dt[Name != "Int. Shipping", ], edgar_emission_dataset_dt_copy[country == "EU", ]), use.names = T)
...
for (sector_i in c("Transportation", "Residential", "Power", "Industry")) {
  ...
  temperature_global_before_2022 <- temperature_global[myear <= 2023, ]
  ...
  Spring_festival_list <- foreach(y = 1970:2023) %do% {
  ...
  fwrite(temperature_global_before_2022, paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/train_and_predict_dataset_", sector_i, "_1229.csv"))
}

merged_data <- fread("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/train_and_predict_dataset_Power_1229.csv")
