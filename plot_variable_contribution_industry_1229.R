...
train_and_predict_dataset <- fread("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/train_and_predict_dataset_Industry_1229.csv")
...
param_table <- fread("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/param_table_industry_1229.csv")
country_list <- c('United States', 'China', 'India', 'Japan', 'Brazil', 
                  'Germany', 'Italy', 'United Kingdom', 'Spain', 'France', 'Russian Federation', 'ROW', "EU")
...
out.data.list <- foreach (country_i = country_list) %do% {
  ...
  fwrite(shap_long, paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/model_shap_industry/", country_i, ".csv"))
  ## 重构的排放
...
global_data_daily <- out.data.all[country %in% c('United States', 'China', 'India', 'Japan', 'Brazil', 'Russian Federation', 'ROW', "EU"), .(tem = mean(tem), 
                                      CM_new = sum(CM_new), 
...
fwrite(out.data.all, "/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/merged_all_new_industry_1128.csv")
...
p.list <- foreach(i = 1:length(country_list)) %do% {
  SHAP_value <- fread(paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/model_shap_industry/", country_list[i], ".csv"))
...
tiff(paste0("/data/ltao/Qiu_paper/plots/temperature_weekly_contribution_industry.tif"), width = 6000, height = 6000, res = 1000, compression = "lzw")
print(p.all)
dev.off()
