...
era5_data_dir <- "/data/ltao/ERA5/country_level_temperature_ROW/"
...
countries_tem_date_y_list <- foreach (y = year_list) %do% {
  if (y < 2000) {
    ...
    r_pop <- terra::aggregate(r_pop, fact = c(15, 15), fun="sum")
  ...
  if (y >= 2000) {
    ...
    r_pop_den <- terra::aggregate(r_pop_den, fact = c(15, 15), fun="mean")
...
fwrite(countries_tem_date_y_all, "/data/ltao/ERA5/countries_population_weigted_mean_temperature_ROW_new_pop.csv")
