...
train_model <- function(train.data, save.dir=".", if.save.model=FALSE, if.var.imp=FALSE, 
  ...
  wY <- train.data[, "w.all"]
  dtrain <- xgb.DMatrix(as.matrix(obsX), label=obsY, weight=wY, nthread=1)
...
train_and_predict_dataset <- fread("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/train_and_predict_dataset_Transportation_1229.csv")
# train_and_predict_dataset[country == "Int", ]
...
param_table <- fread("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/param_table_transportation_1229.csv")
country_list <- c('United States', 'China', 'India', 'Japan', 'Brazil', 
                  'Germany', 'Italy', 'United Kingdom', 'Spain', 'France', 'Russian Federation', 'ROW', "EU", "Int")
...
out.data.list <- foreach (country_i = country_list) %do% {
  ...
  fwrite(shap_long, paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/model_shap_transportation/", country_i, ".csv"))
  ## 重构的排放
...
global_data_daily <- out.data.all[country %in% c('United States', 'China', 'India', 'Japan', 'Brazil', 'Russian Federation', 'ROW', "EU", "Int"), .(tem = mean(tem), 
                                      CM_new = sum(CM_new), 
...
fwrite(out.data.all, "/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/merged_all_new_transportation_1128.csv")
...
p.list <- foreach(i = 1:length(country_list)) %do% {
  SHAP_value <- fread(paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/model_shap_transportation/", country_list[i], ".csv"))
...
tiff(paste0("/data/ltao/Qiu_paper/plots/temperature_weekly_contribution_transportation.tif"), width = 6000, height = 6000, res = 1000, compression = "lzw")
print(p.all)
dev.off()
