...
param_table <- fread("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/param_table_power_1229.csv")
...
out.data.list <- foreach (country_i = country_list) %do% {
  ...
  fwrite(shap_long, paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/model_shap_power/", country_i, ".csv"))
  ## 重构的排放
...
p.list <- foreach(i = 1:length(country_list)) %do% {
  SHAP_value <- fread(paste0("/data/ltao/Qiu_paper/data_for_reconstruction_by_sector/model_shap_power/", country_list[i], ".csv"))
...
tiff(paste0("/data/ltao/Qiu_paper/plots/temperature_weekly_contribution_power.tif"), width = 6000, height = 6000, res = 1000, compression = "lzw")
print(p.all)
dev.off()
